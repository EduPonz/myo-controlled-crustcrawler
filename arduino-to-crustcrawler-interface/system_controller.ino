...
void loop(){
	...
	if (mode == dynamixel.PRE_SET_MODE){
		...
		if (instruction == dynamixel.EXTENDED || instruction == dynamixel.TO_USER || instruction == dynamixel.HOME){
			...
			get_servo_positions();
			path_status = path_planning.calculate_path(instruction, servo_position[0], servo_position[1], servo_position[2]);
			send_json();
			if (path_status == true){
				prev_time = millis();
				while true{
					...
					if (time >= sampling_rate || time == 0){
						for (int i = 0; i < 3; ++i){
							servo_error_pos[i] = convert.position_degrees_to_radians(convert.unit_to_degree(dynamixel.get_current_position(i)) 
											   - path_planning.get_positionSample(i, time)) * k_p;
							servo_error_vel[i] = convert.position_degrees_to_radians(convert.unit_to_degree(dynamixel.get_current_velocity(i)) 
											   - path_planning.get_velocitySample(i, time)) * k_v;
...
void applyInstructions(String instructions) {
	...
	mode = jsonInstructions["mode"].toInt();
	gesture = jsonInstructions["gesture"];
}
