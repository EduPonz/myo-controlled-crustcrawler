...
void PathPlanning::_calculate_acceleration() {
	...
	for (int i = 0; i < 3; i++) {
		...
		if (this->_servo_current_pos[0] > this->_servo_goal_pos[i]) {
...
float PathPlanning::get_position_sample(int servo_id, float sampleTime_milliSec) {
     ...
     if(sampleTime > (this->totalTime - this->_TIME_OF_ACCELERATION) && sampleTime > this->_TIME_OF_ACCELERATION){
...
void PathPlanning::_calculate_deceleration_segment_start_theta() {
	...
	for (int i = 0; i < 3; i++) {
		if (this->constantVelocityTime > 0) {
			segmentStartTheta[i] = this->servo_acceleration[i] * this->_TIME_OF_ACCELERATION * this->constantVelocityTime;
...
bool PathPlanning::calculate_path(float servo1_current_pos, float servo2_current_pos, float servo3_current_pos, String instruction){
	...
	if (this->starting_angle_warning(servo1_current_pos, servo1_current_pos, servo1_current_pos)) {
		...
		this->totalTime = time_total(this->dominant_servo_deltaTheta());
