...
void DynamixelPro2::initialization(){
  ...
  this->write_holding_torque(true);
  this->_change_parameters(10, 10);
  for (int i = 0; i < 3; i++){
...
void DynamixelPro2::write_profile_acceleration(int servo_id, unsigned int pac){
  ...
  this->_write_to_servo_id(servo_id, 0x6C, arr, 4); // 0x6C
}

void DynamixelPro2::write_profile_velocity(int servo_id, unsigned int positionvalue){

  positionvalue %= 1023;

  unsigned char arr[] = {
    (positionvalue & 0xFF),
    (positionvalue & 0xFF00) >> 8,
    (positionvalue & 0xFF0000) >> 16,
    (positionvalue & 0xFF000000) >> 24
  };

  this->_write_to_servo_id(servo_id, 0x70, arr, 4); //max velocity limit 0x70
}
...
void DynamixelPro2::switching_operating_mode(int modeChoice){
  ...
  for (int i = 3; i < 5; i++) {
	  this->write_profile_acceleration(_SERVO_HEX_ID[i], 25);
	  this->write_profile_velocity(_SERVO_HEX_ID[i], 25);
  }
}
...
void DynamixelPro2::_change_parameters(int vel, int acc){
  ...
  for (int i = 0; i < 5; i++){
    this->write_profile_acceleration(this->_SERVO_HEX_ID [i], acc);		// Set the Profile acceleration.(MAX 32767)
    this->write_profile_velocity(this->_SERVO_HEX_ID [i], vel);			// Set the Profile velocity. (MAX 1023)
  }
}
