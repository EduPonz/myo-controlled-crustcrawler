...
int UnitsConverter::torque_to_PWM_unit(int servo_id, float vel, float torque) {	
	...
	int goalPWM = ((((((torque + B*vel) / k)*R) + k*vel) / 12.5) * 885)*(2.1/1.7);
  if(goalPWM > 5 || goalPWM < 5){
    goalPWM += 5; 
    }
...
int UnitsConverter::acceleration_degree_to_unit(float acc_degree) {
	return(acc_degree / _UNIT_TO_ACC_DEGREE);
}
...
float UnitsConverter::speed_degree_to_unit(float speed_degree) {
	return(speed_degree / _UNIT_TO_SPEED_DEGREE);
}
